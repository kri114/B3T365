...
**On Render (free tier, no Shell available):** set the Start Command to run the
idempotent boot script instead — it creates the tables with plain SQL on every
boot and needs no interactive prompts:

```
Build Command:  npm install && npm run build
Start Command:  node scripts/ensure-db.mjs && npm run start
```

`scripts/ensure-db.mjs` mirrors `src/db/schema.ts` and prints masked diagnostics
(`[db] …`) into the Render logs so connection problems are immediately readable.
...
* **Single-device rule:** every time the admin signs in, all previous admin sessions are revoked —
  only the most recent login stays active. Sharing the admin login simply kicks the other device out.
