...
type Action = "credit" | "debit" | "ban" | "unban" | "setlimit";
  ...
  try {
    switch (action) {
      ...
      case "debit": {
        ...
        // Admin deductions may take a player below €0 (fines / clawbacks).
        const balance =
          action === "credit"
            ? await creditUser(id, amount, kind, label, admin.id)
            : await debitUser(id, amount, kind, label, admin.id, true);
        return Response.json({ ok: true, balanceCents: balance });
      }
      case "setlimit": {
        const limit = Math.round(Number(body?.amountCents ?? 0));
        if (!Number.isFinite(limit) || limit < 0 || limit > 100_000_000_00)
          return Response.json({ error: "Overdraft must be zero or a positive amount." }, { status: 400 });
        await db.update(users).set({ creditLimitCents: limit }).where(eq(users.id, id));
        return Response.json({ ok: true, creditLimitCents: limit });
      }
      case "ban": {
