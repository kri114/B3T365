...
const TABS = [
  ...
  { id: "safety", label: "Reports & Passwords", icon: Flag },
  { id: "news", label: "Announcements", icon: Megaphone },
] as const;

type ReportRow = {
  report: { id: string; reason: string; context: string | null; status: string; createdAt: string; reporterId: string };
  reportedName: string;
  reportedId: string;
...
type ResetRow = {
  id: string; userId: string; username: string; note: string | null;
...
export function AdminConsole() {
  ...
  const loadSafety = useCallback(async () => {
    try {
      const [r, p] = await Promise.all([
        fetch("/api/reports", { cache: "no-store" }).then((x) => x.json()),
        fetch("/api/admin/password-resets", { cache: "no-store" }).then((x) => x.json()),
      ]);
      setReports(r.reports ?? []);
      setResets(p.requests ?? []);
    } catch { /* keep */ }
  }, []);
  ...
  useEffect(() => {
    loadUsers(); loadBets(); loadNews(); loadDebts(); loadSafety();
    const t = setInterval(() => { loadUsers(); loadBets(); loadDebts(); loadSafety(); }, 20_000);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {tab === "safety" && (
        <SafetyPanel reports={reports} resets={resets}
          reload={async () => { await loadSafety(); await loadUsers(); }}
          onBan={(id, name) => userAction(id, { action: "ban", note: "Banned after a report" }, `${name} banned`)} />
      )}
        ...
        <table className="w-full min-w-[760px] text-left">
          <thead>
            <tr className="border-b border-line bg-raise/60 text-[10px] font-bold tracking-wider text-faint uppercase">
              ...
              <th className="px-4 py-2.5">Overdraft</th>
          ...
          <tbody>
            {filtered.map((u) => (
              <tr key={u.id} className="border-b border-line/50 align-middle last:border-0 hover:bg-raise/30">
                ...
                <td className={clsx(
                  "tnum px-4 py-3 text-[14px] font-bold",
                  u.balanceCents < 0 ? "text-loss" : "text-gold",
                )}>
                  {formatEuro(u.balanceCents)}
                </td>
                <td className="px-4 py-3">
                  <OverdraftCell user={u} onAction={onAction} />
                </td>
...
/** Per-player overdraft: how far below €0 they may stake. 0 = never negative. */
function OverdraftCell({
  user, onAction,
}: {
  user: AdminUser;
  onAction: (id: string, body: Record<string, unknown>, okMsg: string) => Promise<void>;
}) {
  const [value, setValue] = useState((user.creditLimitCents / 100).toFixed(2));
  const [dirty, setDirty] = useState(false);

  return (
    <div className="flex items-center gap-1">
      <div className="relative">
        <span className="absolute top-1/2 left-2 -translate-y-1/2 text-[11px] text-faint">€</span>
        <input
          value={value}
          onChange={(e) => { setValue(e.target.value); setDirty(true); }}
          inputMode="decimal"
          className="tnum w-20 rounded-lg border border-line bg-raise py-1.5 pr-1.5 pl-5 text-[12px] text-cream outline-none focus:border-gold/50"
        />
      </div>
      {dirty && (
        <button
          onClick={() => {
            ...
            setDirty(false);
            onAction(user.id, { action: "setlimit", amountCents: cents },
              `${user.username} overdraft set to ${formatEuro(cents)}`);
          }}
          className="rounded-md bg-gold/15 px-2 py-1 text-[10.5px] font-bold text-gold hover:bg-gold/25"
        >
          Save
        </button>
      )}
...
function SafetyPanel({
  reports, resets, reload, onBan,
}: {
  reports: ReportRow[];
  resets: ResetRow[];
  reload: () => Promise<void>;
  onBan: (id: string, name: string) => Promise<void>;
}) {
  const { toast } = useToast();
  ...
  const resolve = async (id: string, status: "actioned" | "dismissed") => {
    ...
    toast({ title: `Report ${status}`, tone: "ok" });
    await reload();
  };

  const decideReset = async (id: string, action: "approve" | "reject") => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok) toast({ title: "Failed", body: data?.error, tone: "err" });
    else toast({ title: action === "approve" ? "Password reset approved" : "Reset rejected", tone: "ok" });
    await reload();
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <p className="mb-2 flex items-center gap-1.5 text-[11px] font-bold tracking-wider text-faint uppercase">
          <KeyRound size={11} /> Password reset requests ({pendingResets.length})
        </p>
        {pendingResets.length === 0 ? (
          <p className="rounded-xl border border-dashed border-line py-8 text-center text-[12px] text-faint">
            No pending password resets.
          </p>
        ) : (
          <div className="flex flex-col gap-2">
            {pendingResets.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center gap-2 rounded-xl border border-sky/25 bg-panel p-3.5">
                <span className="text-[13px] font-bold text-cream">{r.username}</span>
                <span className="text-[11px] text-mute">
                  wants a new password{r.note ? ` — “${r.note}”` : ""}
                </span>
                <span className="tnum ml-auto text-[10px] text-faint">
                  {new Date(r.createdAt).toLocaleString([], { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                </span>
                <div className="flex w-full gap-1.5 sm:w-auto">
                  <button onClick={() => decideReset(r.id, "approve")}
                    className="flex items-center gap-1 rounded-lg bg-live/15 px-3 py-1.5 text-[11.5px] font-bold text-live hover:bg-live/25">
                    <Check size={12} /> Approve
                  </button>
                  <button onClick={() => decideReset(r.id, "reject")}
                    className="flex items-center gap-1 rounded-lg bg-loss/15 px-3 py-1.5 text-[11.5px] font-bold text-loss hover:bg-loss/25">
                    <X size={12} /> Reject
                  </button>
                </div>
        ...
        <p className="mt-1.5 text-[10.5px] text-faint">
          Approving applies the password the player chose and signs them out everywhere.
        </p>
      </div>

      <div>
        <p className="mb-2 flex items-center gap-1.5 text-[11px] font-bold tracking-wider text-faint uppercase">
          <Flag size={11} /> Player reports ({openReports.length} open)
        </p>
        {reports.length === 0 ? (
          <p className="rounded-xl border border-dashed border-line py-8 text-center text-[12px] text-faint">
            No reports filed.
          ...
          <div className="flex flex-col gap-2">
            {reports.map(({ report: r, reportedName, reportedId, reportedBanned, reporterName }) => (
              <div key={r.id} className={clsx(
                "rounded-xl border bg-panel p-3.5",
                r.status === "open" ? "border-loss/25" : "border-line",
              )}>
                <div className="flex flex-wrap items-center gap-2">
                  <span className={clsx("rounded-full px-2 py-0.5 text-[9.5px] font-bold uppercase",
                    r.status === "open" ? "bg-loss/15 text-loss" : "bg-lift text-faint")}>
                    {r.status}
                  </span>
                  <span className="text-[12.5px] text-mute">
                    <b className="text-cream">{reporterName}</b> reported <b className="text-cream">{reportedName}</b>
                  </span>
                  {reportedBanned && (
                    <span className="rounded-full bg-loss/15 px-2 py-0.5 text-[9.5px] font-bold text-loss uppercase">banned</span>
                  )}
                  <span className="tnum ml-auto text-[10px] text-faint">
                    {new Date(r.createdAt).toLocaleDateString([], { day: "numeric", month: "short" })}
                  </span>
                </div>
                <p className="mt-1.5 text-[12px] leading-relaxed text-cream/85">“{r.reason}”</p>
                {r.status === "open" && (
                  <div className="mt-2.5 flex flex-wrap gap-1.5">
                    {!reportedBanned && (
                      <button onClick={async () => { await onBan(reportedId, reportedName); await resolve(r.id, "actioned"); }}
                        className="rounded-lg bg-loss/15 px-3 py-1.5 text-[11.5px] font-bold text-loss hover:bg-loss/25">
                        Ban {reportedName}
                      </button>
                    )}
                    <button onClick={() => resolve(r.id, "actioned")}
                      className="rounded-lg bg-lift px-3 py-1.5 text-[11.5px] font-bold text-mute hover:text-cream">
                      Mark actioned
                    </button>
