...
import {
  ...
  ShieldCheck, Search, CircleDollarSign, Ticket, Activity,
} from "lucide-react";
...
export function AdminConsole() {
  ...
  useEffect(() => {
    loadUsers(); loadBets(); loadNews();
    const t = setInterval(() => { loadUsers(); loadBets(); }, 20_000);
  ...
  const stats = useMemo(() => {
    ...
    return { totalBalances, openCount: open.length, liability, staked };
  }, [users, bets]);
