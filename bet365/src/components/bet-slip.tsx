...
export function BetSlip() {
  ...
  const place = async () => {
    if (!user) return;
    // Players may stake into the red down to their overdraft allowance.
    const spendable = user.balanceCents + (user.creditLimitCents ?? 0);
    if (spendable < (mode === "multi" ? multiStakeCents : singlesTotalStake)) {
      toast({
        title: "Stake too high",
        body: `You can stake up to ${formatEuro(Math.max(0, spendable))} including your overdraft.`,
        tone: "err",
      });
      return;
    }
          ...
          <div className="border-t border-line px-4 py-3.5">
            ...
            {user && (
              <p className="tnum mt-2 text-center text-[11px] text-faint">
                Balance:{" "}
                <span className={user.balanceCents < 0 ? "font-bold text-loss" : ""}>
                  {formatEuro(user.balanceCents)}
                </span>
                {(user.creditLimitCents ?? 0) > 0 && (
                  <> · overdraft {formatEuro(user.creditLimitCents ?? 0)}</>
                )}
              </p>
            )}
          </div>
