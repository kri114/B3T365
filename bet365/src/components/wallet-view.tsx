  ...
  return (
    <div className="flex flex-col gap-5">
      <section className="relative overflow-hidden rounded-2xl border border-gold/20 bg-gradient-to-br from-lift via-panel to-panel p-6">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            ...
            <p className={clsx(
              ...
              user.balanceCents < 0 ? "text-loss" : "text-gold",
            )}>
              {formatEuro(user.balanceCents)}
            </p>
            {(user.creditLimitCents ?? 0) > 0 && (
              <p className="tnum mt-0.5 text-[11px] text-faint">
                Overdraft {formatEuro(user.creditLimitCents ?? 0)} · can stake up to{" "}
                <span className="font-bold text-mute">
                  {formatEuro(Math.max(0, user.balanceCents + (user.creditLimitCents ?? 0)))}
                </span>
              </p>
            )}
          </div>
        ...
        {user.balanceCents < 0 && (
          <p className="relative mt-3 rounded-lg border border-loss/30 bg-loss/10 px-3 py-2 text-[11.5px] text-loss">
            You are <b>{formatEuro(Math.abs(user.balanceCents))}</b> in the red. Winning bets pay
            back into this balance first — or ask the house for credit.
          </p>
        )}
