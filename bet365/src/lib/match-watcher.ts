...
export function startMatchWatcher(): void {
  ...
  globalForWatch.__matchWatcherTimer = setInterval(() => {
    runMatchWatch().catch((err) => console.warn("[watch]", (err as Error)?.message ?? err));
  }, 30_000);
  console.log("[watch] match watcher started (30s interval)");
}
