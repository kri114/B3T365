...
export async function placeBet(
  ...
  return await db.transaction(async (tx) => {
    // Atomic balance deduction. Players may stake into the red down to their
    // personal overdraft (credit_limit_cents), so a losing run can leave the
    // balance negative — the house extends credit rather than blocking play.
    const debit = await tx.execute(
      sql`UPDATE users SET balance_cents = balance_cents - ${stakeCents}
          WHERE id = ${userId}
            AND (balance_cents - ${stakeCents}) >= -credit_limit_cents
            AND is_banned = false
    ...
    if (!row) {
      const [u] = await tx
        .select({ balance: users.balanceCents, limit: users.creditLimitCents })
        .from(users)
        .where(eq(users.id, userId))
        .limit(1);
      const available = Math.max(0, (u?.balance ?? 0) + (u?.limit ?? 0));
      throw new BettingError(
        available === 0
          ? "You have no funds to bet with. Ask the house to top up your wallet or grant you credit."
          : `Stake is too high — you can bet up to €${(available / 100).toFixed(2)}.`,
      );
    }
...
async function finalizeBet(betId: string): Promise<void> {
  ...
  if (anyLost) {
    ...
    // A losing bet always breaks the streak.
    await db.update(users).set({ winStreak: 0 }).where(eq(users.id, bet.userId));
    return;
  }
  ...
  // Every leg won → pay exactly the potential return the player was quoted
  // (this already includes any same-game correlation discount).
  // If some legs voided, they drop out of the accumulator and we re-price the
  // remaining ones, preserving the same discount ratio that was applied when
  // the bet was struck.
  const allWon = winningLegs.length === legs.length;
  let payout: number;
  let effectiveOdds: number;

  if (allWon) {
    payout = bet.potentialCents;
    effectiveOdds = bet.totalOdds;
  } else {
    const rawAll = legs.reduce((acc, l) => acc * l.odds, 1);
    // Ratio between the quoted price and the naive product of all legs.
    const discount = rawAll > 0 ? bet.totalOdds / rawAll : 1;
    const rawWinning = winningLegs.reduce((acc, l) => acc * l.odds, 1);
    effectiveOdds = Math.max(1.01, Math.round(rawWinning * discount * 100) / 100);
    payout = Math.round(bet.stakeCents * effectiveOdds);
  }
  ...
  // Extend the winning streak (and the personal best).
  await db.execute(
    sql`UPDATE users
 ...
 *
 * `allowNegative` lets the balance fall below zero — used for admin fines and
 * settlement reversals, where the house decides the number regardless of what
...
export async function overrideBet(
  ...
  // Reverse any previous settlement first. These clawbacks may push the
  // balance negative — the player might already have spent the winnings.
  if (bet.status === "won") {
    await debitUser(bet.userId, bet.potentialCents, "bet_refund", `Admin reversal of bet ${betId.slice(0, 8)}`, adminId, true);
  } else if (bet.status === "void") {
    await debitUser(bet.userId, bet.stakeCents, "bet_refund", `Admin reversal of bet ${betId.slice(0, 8)}`, adminId, true);
  }
